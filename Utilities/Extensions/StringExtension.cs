...
namespace Utilities.Extensions
{
	public static class StringExtension
	{
		...
		{
			var tmp = new string(str.Where(o => (!Char.IsLetterOrDigit(o) && !Char.IsWhiteSpace(o))).ToArray());
			foreach (char c in tmp) { str = str.Replace(c, ' '); }
			return str;
		}
		//{ return new string(str.Where(o => (Char.IsLetterOrDigit(o) || Char.IsWhiteSpace(o))).ToArray()); }
		public static string RemoveExtraSpace(this string str)
		...
		public static string ToSlug(this string value)
		{
			...
			Regex WordDelimiters = new Regex(@"[\s—–_]", RegexOptions.Compiled);
			// characters that are not valid
			Regex InvalidChars = new Regex(@"[^a-z0-9\-]", RegexOptions.Compiled);
			// multiple hyphens
			Regex MultipleHyphens = new Regex(@"-{2,}", RegexOptions.Compiled);
		...
		public static string ReplaceSpecialChar(this string str)
		{
			...
			//if (str.Contains("å")) { str = str.Replace("å", "aa"); }
			//if (str.Contains("ø")) { str = str.Replace("ø", "oe"); }
			//if (str.Contains("ð")) { str = str.Replace("ð", "o"); }
			foreach (char c in "àåáâäãåą".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'a'); } }
			foreach (char c in "èéêëę".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'e'); } }
			foreach (char c in "ìíîïı".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'i'); } }
			foreach (char c in "òóôõöøőð".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'o'); } }
			foreach (char c in "ùúûüŭů".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'u'); } }
			foreach (char c in "çćčĉ".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'c'); } }
			foreach (char c in "żźž".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'z'); } }
			foreach (char c in "śşšŝ".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 's'); } }
			foreach (char c in "ñń".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'n'); } }
			foreach (char c in "ýÿ".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'y'); } }
			foreach (char c in "ğĝ".ToCharArray()) { if (str.Contains(c)) { str = str.Replace(c, 'g'); } }
			if (str.Contains("ř")) { str = str.Replace("ř", "r"); }
			if (str.Contains("ł")) { str = str.Replace("ł", "l"); }
			if (str.Contains("đ")) { str = str.Replace("đ", "d"); }
			if (str.Contains("ß")) { str = str.Replace("ß", "s"); }
			if (str.Contains("þ")) { str = str.Replace("þ", "th"); }
			if (str.Contains("ĥ")) { str = str.Replace("ĥ", "h"); }
			if (str.Contains("ĵ")) { str = str.Replace("j", "j"); }
			if (str.Contains("æ")) { str = str.Replace("æ", "ae"); }
			return str;
		}
		...
		public static string ToBase64(this string s, int lenght)
		{
			string base64 = string.Empty;
			var bits = string.Empty;
			foreach (var character in s) { bits += Convert.ToString(character, 2).PadLeft(lenght, '0'); }
			...
			while (octetsTaken < bits.Length)
			{
				...
				while (hextetsTaken < currentOctects.Count())
				{
					...
					if (bitString.Length < 6) { bitString = bitString.PadRight(6, '0'); }
					var singleInt = Convert.ToInt32(bitString, 2);
					base64 += StringHelper.Base64Letters[singleInt];
				}
				octetsTaken += threeOctets;
			}
			// Pad with = for however many octects we have left
			for (var i = 0; i < (bits.Length % 3); i++) { base64 += "="; }
		...
		public static string DecodeFrom64(this string str)
		{
		...
		public static string ScribanParserWithHtmlAgilityPack(this string html)
		{
			var s = -2;
			if (!html.IsNullOrEmpty())
			{
				while (html.IndexOf("{{", s + 2) > -1)
				{
					s = html.IndexOf("{{", s + 2);
					var str = html.Substring(s, html.IndexOf("}}", s) + 2 - s);
					html = html.Replace(str, WebUtility.HtmlDecode(str));
				}
			}
			var doc = new HtmlDocument();
			doc.LoadHtml(html);
			var htmls = doc.DocumentNode.QuerySelectorAll("[tpl-src]");
			for (int i = 0; i < htmls.Count; i++)
			{
				if (htmls[i].GetAttributeValue("tpl-src", "") != "")
				{
					htmls[i].SetAttributeValue("src", htmls[i].GetAttributeValue("tpl-src", ""));
					htmls[i].Attributes.Remove("tpl-src");
				}
			}
			htmls = doc.DocumentNode.QuerySelectorAll("[tpl-for]");
			for (int i = 0; i < htmls.Count; i++)
			{
				if (htmls[i].GetAttributeValue("tpl-for", "") != "")
				{
					HtmlNode newChild = HtmlTextNode.CreateNode("{{for " + htmls[i].GetAttributeValue("tpl-for", "") + "}}");
					(htmls[i]?.ParentNode ?? doc.DocumentNode).InsertBefore(newChild, htmls[i]);
					HtmlNode newChild1 = HtmlTextNode.CreateNode("{{end}}");
					(htmls[i]?.ParentNode ?? doc.DocumentNode).InsertAfter(newChild1, htmls[i]);
				}
				htmls[i].Attributes.Remove("tpl-for");
			}
			var htmlsIf = doc.DocumentNode.QuerySelectorAll("[tpl-if]");
			for (int i = 0; i < htmlsIf.Count; i++)
			{
				if (htmlsIf[i].GetAttributeValue("tpl-if", "") != "")
				{
					HtmlNode newChild = HtmlTextNode.CreateNode("{{if " + htmlsIf[i].GetAttributeValue("tpl-if", "") + "}}");
					(htmlsIf[i]?.ParentNode ?? doc.DocumentNode).InsertBefore(newChild, htmlsIf[i]);
					HtmlNode newChild1 = HtmlTextNode.CreateNode("{{end}}");
					(htmlsIf[i]?.ParentNode ?? doc.DocumentNode).InsertAfter(newChild1, htmlsIf[i]);
				}
				htmlsIf[i].Attributes.Remove("tpl-if");
			}
			...
			{
				if (htmlResult.Contains("<meta_resource"))
				{
					...
					{
						foreach (HtmlNode item in doc.DocumentNode.SelectNodes("//meta_resource"))
						{
							HtmlAttribute keyAtt = item.Attributes["key"];
							if (keyAtt != null && !string.IsNullOrEmpty(keyAtt.Value))
							{
								string _params = " key:'" + keyAtt.Value + "'";
								foreach (var _key in item.Attributes.Where(t => t.Name != "key").ToList())
								{
									_params += " " + _key.Name + ":" + _key.Value;
								}
								var newHeadNode = HtmlNode.CreateNode("{{meta_resource" + _params + "}}");
						...
						htmlResult = doc.DocumentNode.OuterHtml;
					}
				}
				if (htmlResult.Contains("<widget_resource"))
				{
					doc = new HtmlDocument();
					doc.LoadHtml(htmlResult);
					...
					if (doc.DocumentNode != null)
					{
						foreach (HtmlNode item in doc.DocumentNode.SelectNodes("//widget_resource"))
						{
							HtmlAttribute keyAtt = item.Attributes["key"];
							if (keyAtt != null && !string.IsNullOrEmpty(keyAtt.Value))
							{
								...
								var newHeadNode = HtmlNode.CreateNode("{{widget_inline" + _params + "}}");
								item.ParentNode.ReplaceChild(newHeadNode, item);
								countP++;
							}
						}
						htmlResult = doc.DocumentNode.OuterHtml;
